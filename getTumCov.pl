...
# skip comment. read header
my $header = <IN>;
while ($header =~ /^#/) {
  $header = <IN>; 
} 
chomp $header;
print OUT "$header\tTTot$covLabel\tTVar$covLabel\n";
...
while(<IN>) {
  ...
  ###  BAMS
  my $tbam = `ls $bamdir/*.bam | grep $tsam`;
  chomp $tbam; 

  # remove chr if bam ref doesn't have chr
  if($tbam) {
    my $bamheader = `samtools view -H $tbam | grep \@SQ | head -1`;
    if ($bamheader !~ /SN:chr\w+/) {
      $chr =~ s/chr//;
    }
  }	 
